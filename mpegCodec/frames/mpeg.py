				...
				for j in range (0, rC, self.mbc):
					...
					for a in range (i-self.sspace, i+self.sspace):
						for b in range (j-self.sspace, j+self.sspace):
							mae = (1.0/(self.mbr*self.mbc))*(np.sum(cv2.absdiff(self.currentfr[i:i+self.mbr,j:j+self.mbc, 0], pastfr[self.sspace+a:self.sspace+a+self.mbr, self.sspace+b:self.sspace+b+self.mbc,0])))
							if mad >= mae:
								mad = mae
								x = a-i
								y = b-j
			...
			for i in range(0, rR, self.mbr):
				for j in range (0, rC, self.mbc):
					...
					while s >= 1:
						for a in [i-s, i, i+s]:
							for b in [j-s, j, j+s]:
								if a+self.sspace <= self.sspace:
									continue
								if b+self.sspace <= self.sspace:
									continue
								mae = (1.0/(self.mbr*self.mbc))*(np.sum(cv2.absdiff(self.currentfr[i:i+self.mbr,j:j+self.mbc, 0], pastfr[self.sspace+a:self.sspace+a+self.mbr, self.sspace+b:self.sspace+b+self.mbc,0])))
								if mad >= mae:
									mad = mae
...
#					motionVec.append((count,'b',backwardMVec[count][0],backwardMVec[count][1]))
					motionVec.append((count,'b',b0,b1))
				elif w == 1:	# i
...
#					motionVec.append((count,'i',interpolativeMVec[count][0],interpolativeMVec[count][1],interpolativeMVec[count][2],interpolativeMVec[count][3]))
					motionVec.append((count,'i',f0,f1,b0,b1))
				elif w == 2:	#f
...
#					motionVec.append((count,'f',forewardMVec[count][0],forewardMVec[count][1]))
					motionVec.append((count,'f',f0,f1))
				count += 1
